...
const QString program_name = "Clock";             //Change "Desktop" to your app name
//start below
...
//    connect(appFeatures[5], &QAction::triggered, this, &newApp::close_app);
    //start below
    connect(start,&QPushButton::clicked,this,&newApp::stopwatchStart);
    connect(pause,&QPushButton::clicked,this,&newApp::stopwatchPause);
    connect(rst,&QPushButton::clicked,this,&newApp::stopwatchReset);
    startTimer(0);

    connect(startbtn,&QPushButton::clicked,this,&newApp::timerStart);
    connect(resetbtn,&QPushButton::clicked,this,&newApp::timerReset);
}
...
void newApp::stopwatchStart(void){
...
void newApp::stopwatchPause(void){
    start->setEnabled(true);
    pause->setEnabled(false);
    rst->setEnabled(true);
...
void newApp::stopwatchReset(void){
    start->setEnabled(true);
    pause->setEnabled(false);
    rst->setEnabled(false);
    totalTime = 0;
...
void newApp::timerEvent(QTimerEvent *){
    if(running){
...
void newApp::timerStart(){
    startbtn->setEnabled(false);
    resetbtn->setEnabled(true);
    countdown.setHMS(insHour->value(),insMinute->value(),insSecond->value()+1);
    time2 = new QTimer;
    connect(time2,&QTimer::timeout,this,&newApp::updateTimer);
    time2->start(1000);
}

void newApp::timerReset(){
    startbtn->setEnabled(true);
    resetbtn->setEnabled(false);
    time2->stop();
